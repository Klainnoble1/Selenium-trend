...
def send_to_n8n(payload: dict, webhook_url: str | None = None) -> dict:
    ...
    """
    url = (webhook_url or os.environ.get("N8N_WEBHOOK_URL") or "").strip()
    if not url:
        return {"success": False, "error": "N8N_WEBHOOK_URL not set"}

    method = (os.environ.get("N8N_WEBHOOK_METHOD") or "POST").strip().upper()
    access_token = (os.environ.get("N8N_ACCESS_TOKEN") or "").strip()
    auth_header_name = (
        os.environ.get("N8N_WEBHOOK_AUTH_HEADER") or "Authorization"
    ).strip()
    ...
    if access_token:
        if auth_header_name.lower() == "authorization":
            ...
            headers[auth_header_name] = access_token
